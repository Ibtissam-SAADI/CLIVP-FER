...
trainset = RAFDB(split = 'Training', fold = opt.fold, transform=transforms_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=opt.bs, shuffle=True, num_workers=0)
testset = RAFDB(split = 'Testing', fold = opt.fold, transform=transforms_vaild)
...
def train(epoch):
    ...
    for index, (images, captions, labels) in enumerate(trainloader):
        ...
        batch_f1 = f1_score(batch_labels, batch_predictions, average='macro', zero_division=0)
    ...
    average_f1_score = sum(batch_f1_scores) / len(batch_f1_scores)
    print(f'Epoch {epoch}: average_f1_score: {average_f1_score:.4f}')
...
def test(epoch):
    ...
    if Test_acc > best_Test_acc:
        ...
        if not os.path.isdir(path):
            os.mkdir(path)
        torch.save(state, os.path.join(path, 'best_modelrafbd3.t7'))
        best_Test_acc = Test_acc
        best_Test_acc_epoch = epoch
